...
run_test() {
    ...
    if [ $? -eq 0 ]; then
        echo -e "$total_tests ${GREEN}PASS${DEFAULT}: s21_cat $option $option2 $option3 $file $file2"
        return 0
    else
        echo -e "$total_tests ${RED}FAIL${DEFAULT}: s21_cat $option $option2 $option3 $file $file2"
        return 1
    fi
}
